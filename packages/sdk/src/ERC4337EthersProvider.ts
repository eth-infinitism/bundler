...
import { EntryPoint, UserOperationStruct } from '@account-abstraction/utils/src/ContractTypes'
...
export class ERC4337EthersProvider extends AbstractProvider {
  ...
  async _detectNetwork (): Promise<Network> {
    return await this.originalProvider.getNetwork()
  }

  async _perform (req: PerformActionRequest): Promise<any> {
    const keys = Object.keys(req).slice(1)
    const params = keys.map(key => (req as any)[key])
    debug('>> perform', req.method, 'keys=', keys, 'params=', params)
    const ret = await (this.originalProvider as any)[req.method](...params)
    return ret
  ...
  async perform (method: string, params: any): Promise<any> {
    ...
    return (this.originalProvider.provider as any).send(method, params)
  }
